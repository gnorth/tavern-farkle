...
import {diceReplay,type Pose} from '@/lib/dice-replay';
...
function pipTexture(value:number){const canvas=document.createElement('canvas');canvas.width=canvas.height=256;const ctx=canvas.getContext('2d')!;ctx.fillStyle='#d9c9a3';ctx.fillRect(0,0,256,256);
 ...
 for(let y=0;y<256;y+=2)for(let x=0;x<256;x+=2){const n=(Math.sin(x*12.9898+y*78.233+value*7)*43758.5453)%1;ctx.fillStyle=n>0?'rgba(92,62,27,.2)':'rgba(255,250,226,.24)';ctx.fillRect(x,y,2,2);}
 for(let i=0;i<26;i++){const x=(i*47+value*19)%256,y=(i*73)%256;ctx.beginPath();ctx.moveTo(x,y);ctx.bezierCurveTo(x+9,y+6,x-8,y+19,x+3,y+35);ctx.strokeStyle='rgba(92,60,28,.3)';ctx.lineWidth=2.4;ctx.stroke();}
 const edge=ctx.createRadialGradient(128,128,65,128,128,175);edge.addColorStop(0,'rgba(92,62,24,0)');edge.addColorStop(1,'rgba(92,62,24,.38)');ctx.fillStyle=edge;ctx.fillRect(0,0,256,256);const pips:Record<number,number[][]>={1:[[0,0]],2:[[-1,-1],[1,1]],3:[[-1,-1],[0,0],[1,1]],4:[[-1,-1],[-1,1],[1,-1],[1,1]],5:[[-1,-1],[-1,1],[0,0],[1,-1],[1,1]],6:[[-1,-1],[-1,0],[-1,1],[1,-1],[1,0],[1,1]]};pips[value].forEach(([x,y])=>{ctx.beginPath();ctx.arc(128+x*63,128+y*63,value===1?27:23,0,Math.PI*2);ctx.fillStyle=value===1?'#430d08':'#090705';ctx.fill();ctx.beginPath();ctx.arc(126+x*63,126+y*63,14,Math.PI,Math.PI*1.8);ctx.strokeStyle='#100b0766';ctx.lineWidth=3;ctx.stroke();});const t=new THREE.CanvasTexture(canvas);t.colorSpace=THREE.SRGBColorSpace;return t;}
...
type Props={authoritative?:boolean;game:Game;onResult:(v:Record<number,number>,rollId:number)=>void;onSelect:(id:number)=>void;interactive:boolean};
...
#include <opaque_fragment>`);};boardMat.customProgramCacheKey=()=> 'dark-walnut-contrast-v4';const edgeMat=new THREE.MeshStandardMaterial({color:0x714526,roughness:.7});const brass=new THREE.MeshStandardMaterial({color:0xd5a14d,metalness:.65,roughness:.48});
  ...
  function reset(){replay=null;parkedIndices=[];clearAt=null;meshes.forEach(restoreDie);transfers=[];pendingLaunch=null;visualSelected=[];rolling=false;bodies.forEach((b,i)=>{clearShellRotation(b);b.type=CANNON.Body.STATIC;b.collisionResponse=true;b.position.set((i%3-1)*1.8,.51*dieScale,(Math.floor(i/3)-.5)*1.8);b.quaternion.setFromEuler(0,0,0);const target=normals[faceValues.indexOf(latest.current.authoritative?latest.current.game.dice[i]:i+1)];b.quaternion.setFromVectors(target,new CANNON.Vec3(0,1,0));b.velocity.setZero();b.angularVelocity.setZero();b.updateMassProperties();b.previousPosition.copy(b.position);b.interpolatedPosition.copy(b.position);b.previousQuaternion.copy(b.quaternion);b.interpolatedQuaternion.copy(b.quaternion);b.aabbNeedsUpdate=true;});}
  ...
  function clear(){
   ...
   clearAt=performance.now();rolling=false;pendingLaunch=null;transfers=[];visualSelected=[];
  ...
  function beginThrow(ids:number[],id:number){parkedIndices=parkedIndices.filter(i=>!ids.includes(i));clearAt=null;ids.forEach(i=>restoreDie(meshes[i]));active=ids;rollId=id;start=performance.now();lastNudge=start;rolling=true;if(latest.current.authoritative){
    replay=diceReplay(ids.map(i=>latest.current.game.dice[i]),latest.current.game.replaySeed??id,dieScale);
    ids.forEach(i=>{const b=bodies[i];clearShellRotation(b);b.type=CANNON.Body.STATIC;b.collisionResponse=false;b.velocity.setZero();b.angularVelocity.setZero();});start=performance.now();
   }else{replay=null;ids.forEach((i,j)=>{bodies[i].collisionResponse=true;launchDie(bodies[i],j,undefined,phone.matches);});}}
  function roll(ids:number[],id:number){
   ...
   parkedIndices=toPark;const now=performance.now();
   toPark.forEach((i,slot)=>{const b=bodies[i],mesh=meshes[i];const to=parkedPosition(slot,toPark.length);const q=new CANNON.Quaternion();q.setFromVectors(normals[faceValues.indexOf(latest.current.game.dice[i])],new CANNON.Vec3(0,1,0));
  ...
  const resize=()=>{
   ...
   if(phone.matches&&!portrait){
    // Fixed side rails match landscape CSS; control count cannot resize the arena.
    const left=160,right=196,availableWidth=Math.max(100,w-left-right);
    const scale=Math.min(availableWidth/6.2,Math.max(100,h-42)/7.4);
    vertical=h/scale;
    const centerX=(w/2-(left+availableWidth/2))/scale;
    camera.position.set(centerX,20,-.65);camera.lookAt(centerX,0,-.65);
   }else if(phone.matches){
    const styles=getComputedStyle(host.parentElement!);
    const safeTop=parseFloat(styles.getPropertyValue('--safe-top'))||8;
    const safeBottom=parseFloat(styles.getPropertyValue('--safe-bottom'))||14;
    // Reserve the largest control state; selection must never change camera zoom.
    const top=safeTop+(portrait?143:137);
    const bottom=safeBottom+(portrait?190:138);
  ...
  let raf=0,last=performance.now();function frame(now:number){const dt=Math.min((now-last)/1000,.05);last=now;if(pendingLaunch&&now>=pendingLaunch.at){const next=pendingLaunch;pendingLaunch=null;transfers=[];beginThrow(next.ids,next.id);}world.step(PHYSICS_STEP,dt,8);
   if(replay){const progress=Math.min(1,(now-start)/1800),index=progress*(replay.length-1),a=Math.floor(index),b=Math.min(a+1,replay.length-1);active.forEach((id,j)=>{const body=bodies[id];replay![a][j].p.lerp(replay![b][j].p,index-a,body.position);replay![a][j].q.slerp(replay![b][j].q,index-a,body.quaternion);syncBody(body);});if(progress===1){replay=null;rolling=false;latest.current.onResult(Object.fromEntries(active.map(i=>[i,latest.current.game.dice[i]])),rollId);}}
   bodies.forEach((b,i)=>{meshes[i].position.copy(b.interpolatedPosition as unknown as THREE.Vector3);meshes[i].quaternion.copy(visibleQuaternion(b,true) as unknown as THREE.Quaternion);const transfer=transfers.find(t=>t.index===i);if(transfer){const t=transfer.duration?Math.min(1,Math.max(0,(now-transfer.start)/transfer.duration)):1;const eased=t*t*(3-2*t);meshes[i].position.lerpVectors(transfer.from,transfer.to,eased);meshes[i].position.y+=Math.sin(Math.PI*t)*.22;meshes[i].quaternion.slerpQuaternions(transfer.fromQ,transfer.toQ,eased);}if(clearAt!==null){const t=reducedMotion?1:Math.min(1,Math.max(0,(now-clearAt-200-i*35)/450));meshes[i].material.forEach(m=>m.opacity=1-t);meshes[i].scale.setScalar(1-t*.15);meshes[i].castShadow=t===0;meshes[i].visible=t<1;rings[i].visible=false;}rings[i].position.set(meshes[i].position.x,.06,meshes[i].position.z);const el=buttons.current[i];if(el){el.style.visibility=meshes[i].visible?'visible':'hidden';const p=meshes[i].position.clone().project(camera);el.style.left=`${(p.x*.5+.5)*host.clientWidth}px`;el.style.top=`${(-p.y*.5+.5)*host.clientHeight}px`;const size=Math.max(44,host.clientWidth/(camera.right-camera.left)*1.4);el.style.width=el.style.height=`${size}px`;}});
   if(rolling&&!replay&&now-start>650){const stable=active.every(i=>bodies[i].sleepState===CANNON.Body.SLEEPING);const flat=active.every(i=>upperFace(bodies[i].quaternion).alignment>.985&&bodies[i].position.y<.65);
    if(stable&&flat){
     rolling=false;const values:Record<number,number>={};active.forEach(i=>values[i]=rolledFace(bodies[i]).value);latest.current.onResult(values,rollId);}
    else if(now-lastNudge>750){active.forEach(i=>nudgeTilted(bodies[i]));lastNudge=now;}
 ...
 useEffect(()=>{if(props.game.phase==='rolling')controller.current?.roll(props.game.dice.map((_,i)=>i).filter(i=>!props.game.locked.includes(i)),props.game.rollId);else if(['bust','handoff','won'].includes(props.game.phase))controller.current?.clear();else if(props.game.phase==='ready'&&props.game.round===1&&props.game.player===0)controller.current?.reset();},[props.game.rollId,props.game.phase]);
