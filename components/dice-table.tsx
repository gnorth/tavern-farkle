...
diffuseColor.rgb = mix(vec3(woodLuminance), diffuseColor.rgb, 0.25) * 0.42;`);};boardMat.customProgramCacheKey=()=> 'muted-oak-v1';const edgeMat=new THREE.MeshStandardMaterial({color:0x714526,roughness:.7});const brass=new THREE.MeshStandardMaterial({color:0xd5a14d,metalness:.65,roughness:.48});
  ...
  function reset(){rolling=false;bodies.forEach((b,i)=>{b.type=CANNON.Body.STATIC;b.position.set((i%3-1)*1.8,.51,(Math.floor(i/3)-.5)*1.8);b.quaternion.setFromEuler(0,0,0);const target=normals[faceValues.indexOf(i+1)];b.quaternion.setFromVectors(target,new CANNON.Vec3(0,1,0));b.velocity.setZero();b.angularVelocity.setZero();b.updateMassProperties();});}
  for(let i=0;i<6;i++){const material=textures.map(map=>new THREE.MeshStandardMaterial({map,roughness:.58,metalness:0,emissive:0x000000}));const mesh=new THREE.Mesh(geometry,material);mesh.castShadow=mesh.receiveShadow=true;mesh.userData.index=i;scene.add(mesh);meshes.push(mesh);const ring=new THREE.Mesh(new THREE.TorusGeometry(.77,.055,8,56),new THREE.MeshBasicMaterial({color:0xffd238,depthTest:false}));ring.rotation.x=-Math.PI/2;ring.renderOrder=2;ring.visible=false;scene.add(ring);rings.push(ring);const b=createDie(world);bodies.push(b);}
  reset();
  ...
  function highlight(){rings.forEach((r,i)=>r.visible=latest.current.game.selected.includes(i));meshes.forEach((m,i)=>m.material.forEach(mat=>{mat.emissive.setHex(latest.current.game.selected.includes(i)?0x947025:0x000000);mat.emissiveIntensity=.55;mat.color.setHex(latest.current.game.locked.includes(i)?0xb4a78d:0xffffff);}));}
  controller.current={roll,highlight,reset};
  const resize=()=>{const w=host.clientWidth,h=host.clientHeight;renderer.setSize(w,h);const aspect=w/Math.max(h,1);const portrait=aspect<.85;camera.up.set(portrait?1:0,0,portrait?0:-1);camera.lookAt(0,0,0);const vertical=portrait?Math.max(20.5,10.8/aspect):Math.max(10,14/aspect);camera.left=-vertical*aspect/2;camera.right=vertical*aspect/2;camera.top=vertical/2;camera.bottom=-vertical/2;camera.updateProjectionMatrix();};const observer=new ResizeObserver(resize);observer.observe(host);resize();
  ...
  let raf=0,last=performance.now();function frame(now:number){const dt=Math.min((now-last)/1000,.05);last=now;world.step(1/60,dt,4);bodies.forEach((b,i)=>{meshes[i].position.copy(b.position as unknown as THREE.Vector3);meshes[i].quaternion.copy(b.quaternion as unknown as THREE.Quaternion);rings[i].position.set(b.position.x,.06,b.position.z);const el=buttons.current[i];if(el){const p=meshes[i].position.clone().project(camera);el.style.left=`${(p.x*.5+.5)*host.clientWidth}px`;el.style.top=`${(-p.y*.5+.5)*host.clientHeight}px`;const size=Math.max(44,host.clientWidth/(camera.right-camera.left)*1.4);el.style.width=el.style.height=`${size}px`;}});
 ...
 return <><div ref={mount} className="dice-canvas"/>{props.game.dice.map((v,i)=><button key={i} ref={el=>{buttons.current[i]=el;}} className="dice-hit" aria-label={`Кубик ${i+1}: ${v}${props.game.locked.includes(i)?", відкладений":""}`} aria-pressed={props.game.selected.includes(i)} disabled={!props.interactive||props.game.locked.includes(i)} onClick={()=>props.onSelect(i)}/>)}{error&&<div className="graphics-error" role="alert">Не вдалося запустити 3D. Спробуйте браузер із підтримкою WebGL або ввімкніть апаратне прискорення.</div>}</>;
}
