...
#include <opaque_fragment>`);};boardMat.customProgramCacheKey=()=> 'dark-walnut-contrast-v4';const edgeMat=new THREE.MeshStandardMaterial({color:0x714526,roughness:.7});const brass=new THREE.MeshStandardMaterial({color:0xd5a14d,metalness:.65,roughness:.48});
  ...
  function roll(ids:number[],id:number){active=ids;rollId=id;start=performance.now();lastNudge=start;rolling=true;
   bodies.forEach((b,i)=>{if(!ids.includes(i)){b.type=CANNON.Body.STATIC;b.position.set((i-2.5)*1.45,.52,-3.35);b.velocity.setZero();b.angularVelocity.setZero();b.updateMassProperties();b.previousPosition.copy(b.position);b.interpolatedPosition.copy(b.position);b.previousQuaternion.copy(b.quaternion);b.interpolatedQuaternion.copy(b.quaternion);b.aabbNeedsUpdate=true;return;}
