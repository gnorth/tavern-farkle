...
function pipTexture(value:number){const canvas=document.createElement('canvas');canvas.width=canvas.height=256;const ctx=canvas.getContext('2d')!;ctx.fillStyle='#eadfbd';ctx.fillRect(0,0,256,256);const pips:Record<number,number[][]>={1:[[0,0]],2:[[-1,-1],[1,1]],3:[[-1,-1],[0,0],[1,1]],4:[[-1,-1],[-1,1],[1,-1],[1,1]],5:[[-1,-1],[-1,1],[0,0],[1,-1],[1,1]],6:[[-1,-1],[-1,0],[-1,1],[1,-1],[1,0],[1,1]]};pips[value].forEach(([x,y])=>{ctx.beginPath();ctx.arc(128+x*63,128+y*63,value===1?24:19,0,Math.PI*2);ctx.fillStyle=value===1?'#823c29':'#30271f';ctx.fill();ctx.beginPath();ctx.arc(126+x*63,126+y*63,14,Math.PI,Math.PI*1.8);ctx.strokeStyle='#100b0766';ctx.lineWidth=3;ctx.stroke();});const t=new THREE.CanvasTexture(canvas);t.colorSpace=THREE.SRGBColorSpace;return t;}
function woodTexture(onLoad:()=>void){const texture=new THREE.TextureLoader().load('/textures/tavern-oak.jpg',onLoad);texture.colorSpace=THREE.SRGBColorSpace;texture.anisotropy=4;return texture;}
...
export default function DiceTable(props:Props){
 ...
 useEffect(()=>{
  const host=mount.current!;let dirty=true;const mobile=window.matchMedia('(pointer: coarse)').matches;let renderer:THREE.WebGLRenderer;
  ...
  scene.add(new THREE.HemisphereLight(0xffe6c7,0x302031,2.5));const light=new THREE.DirectionalLight(0xffcd86,3.2);light.position.set(-3,16,3);light.castShadow=true;light.shadow.mapSize.set(mobile?512:1024,mobile?512:1024);Object.assign(light.shadow.camera,{left:-9,right:9,top:9,bottom:-9,near:.5,far:25});light.shadow.bias=-.001;scene.add(light);const fill=new THREE.PointLight(0xff8e36,12,20);fill.position.set(7,3,-4);scene.add(fill);
  const world=createWorld();
  const textures=faceValues.map(pipTexture),wood=woodTexture(()=>{dirty=true;});const geometry=new RoundedBoxGeometry(.95,.95,.95,10,DICE_RADIUS);
...
#include <opaque_fragment>`);};boardMat.customProgramCacheKey=()=> 'dark-walnut-contrast-v4';const edgeMat=new THREE.MeshStandardMaterial({color:0x714526,roughness:.7});const brass=new THREE.MeshStandardMaterial({color:0xd5a14d,metalness:.65,roughness:.48});
  ...
  function highlight(){dirty=true;visualSelected=[...latest.current.game.selected];rings.forEach((r,i)=>r.visible=latest.current.game.selected.includes(i));meshes.forEach((m,i)=>m.material.forEach(mat=>{mat.emissive.setHex(latest.current.game.selected.includes(i)?0x947025:0x000000);mat.emissiveIntensity=.12;mat.color.setHex(latest.current.game.locked.includes(i)?0xb4a78d:0xffffff);}));}
  controller.current={roll,highlight,reset,clear};
  const resize=()=>{const w=Math.max(1,host.clientWidth),h=Math.max(1,host.clientHeight);renderer.setSize(w,h);const aspect=w/h;const compact=window.matchMedia('(max-width: 900px), (pointer: coarse)').matches;const portrait=compact?aspect<1:aspect<.85;camera.up.set(portrait?1:0,0,portrait?0:-1);camera.lookAt(0,0,0);const vertical=compact?(portrait?Math.max(13.2,10.8/aspect):Math.max(10.8,13.2/aspect)):(portrait?Math.max(20.5,12.4/aspect):Math.max(12,14/aspect));camera.left=-vertical*aspect/2;camera.right=vertical*aspect/2;camera.top=vertical/2;camera.bottom=-vertical/2;camera.updateProjectionMatrix();dirty=true;};const observer=new ResizeObserver(resize);observer.observe(host);resize();
  ...
  let raf=0,last=performance.now();function frame(now:number){const dt=Math.min((now-last)/1000,.05);last=now;if(document.hidden){raf=requestAnimationFrame(frame);return;}const animating=rolling||pendingLaunch!==null||transfers.some(t=>now<t.start+t.duration)||clearAt!==null&&now<clearAt+900;if(!animating&&!dirty){raf=requestAnimationFrame(frame);return;}dirty=false;if(pendingLaunch&&now>=pendingLaunch.at){const next=pendingLaunch;pendingLaunch=null;transfers=[];beginThrow(next.ids,next.id);}if(rolling)world.step(PHYSICS_STEP,dt,8);bodies.forEach((b,i)=>{meshes[i].position.copy(b.interpolatedPosition as unknown as THREE.Vector3);meshes[i].quaternion.copy(visibleQuaternion(b,true) as unknown as THREE.Quaternion);const transfer=transfers.find(t=>t.index===i);if(transfer){const t=transfer.duration?Math.min(1,Math.max(0,(now-transfer.start)/transfer.duration)):1;const eased=t*t*(3-2*t);meshes[i].position.lerpVectors(transfer.from,transfer.to,eased);meshes[i].position.y+=Math.sin(Math.PI*t)*.22;meshes[i].quaternion.slerpQuaternions(transfer.fromQ,transfer.toQ,eased);}if(clearAt!==null){const t=reducedMotion?1:Math.min(1,Math.max(0,(now-clearAt-200-i*35)/450));meshes[i].material.forEach(m=>m.opacity=1-t);meshes[i].scale.setScalar(1-t*.15);meshes[i].castShadow=t===0;meshes[i].visible=t<1;rings[i].visible=false;}rings[i].position.set(meshes[i].position.x,.06,meshes[i].position.z);const el=buttons.current[i];if(el){el.style.visibility=meshes[i].visible?'visible':'hidden';const p=meshes[i].position.clone().project(camera);el.style.left=`${(p.x*.5+.5)*host.clientWidth}px`;el.style.top=`${(-p.y*.5+.5)*host.clientHeight}px`;const size=Math.max(44,host.clientWidth/(camera.right-camera.left)*1.4);el.style.width=el.style.height=`${size}px`;}});
   if(rolling&&now-start>650){const stable=active.every(i=>bodies[i].sleepState===CANNON.Body.SLEEPING);const flat=active.every(i=>upperFace(bodies[i].quaternion).alignment>.985&&bodies[i].position.y<.65);
    if(stable&&flat){rolling=false;const values:Record<number,number>={};active.forEach(i=>values[i]=rolledFace(bodies[i]).value);latest.current.onResult(values,rollId);}
 ...
 return <div className="dice-stage"><div ref={mount} className="dice-canvas"/>{props.game.dice.map((v,i)=><button key={i} ref={el=>{buttons.current[i]=el;}} className={`dice-hit ${props.game.selected.includes(i)?"is-selected":""}`} aria-label={`Кубик ${i+1}: ${v}${props.game.locked.includes(i)?", відкладений":""}`} aria-pressed={props.game.selected.includes(i)} disabled={!props.interactive||props.game.locked.includes(i)} onClick={()=>props.onSelect(i)}></button>)}{error&&<div className="graphics-error" role="alert">Не вдалося запустити 3D. Спробуйте браузер із підтримкою WebGL або ввімкніть апаратне прискорення.</div>}</div>;
}
