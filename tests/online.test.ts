...
test('only the waiting host can change the target',()=>{const r=room();r.names=['A'];const updated=roomAction(r,0,{type:'target',target:8000});assert.equal(updated.game.target,8000);assert.equal(r.game.target,4000);assert.throws(()=>roomAction(r,1,{type:'target',target:6000}));assert.throws(()=>roomAction(r,0,{type:'target',target:5000}));assert.throws(()=>roomAction(room(),0,{type:'target',target:6000}));});
